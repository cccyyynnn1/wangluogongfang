  ...
  const randomTheme = async () => {
    ...
    theme.value.layout =
      device.value === 'desktop'
        ? shuffle(theme.value.layout, ['horizontal', 'vertical', 'column', 'comprehensive', 'common', 'float'])
        : 'vertical'
      ...
      <el-form ref="form" label-position="left" :model="theme">
        <el-form-item>
          <template #label>
            {{ translate('布局') }}
            <el-tooltip
              :content="translate('布局配置仅' + '在电脑视窗下生效，手机视窗时将' + '默认锁定为纵向布局')"
              effect="dark"
              placement="top"
            >
          ...
          <el-select v-model="theme.layout" :disabled="device === 'mobile'" placeholder="请选择">
            ...
            <el-option key="horizontal" :label="translate('横向')" value="horizontal" />
        ...
        <el-form-item :label="translate('主题')">
          <el-select v-model="theme.themeName" @change="updateTheme">
            ...
            <el-option key="green-white" :label="translate('绿白')" value="green-white" />
            <el-option key="purple-black" :label="translate('紫黑')" value="purple-black" />
            <el-option key="purple-white" :label="translate('紫白')" value="purple-white" />
        ...
        <el-form-item class="vab-item-custom">
          <template #label>
            ...
            <el-tooltip
              :content="
                translate('支持纵向布局' + '、分栏布局、综合' + '布局、常规布局，不支持横' + '向布局、浮动' + '布局')
              "
              effect="dark"
              placement="top"
            >
          ...
          <el-radio-group
            ...
            :disabled="theme.layout === 'float' || theme.layout === 'horizontal'"
            @change="updateTheme"
          >
        ...
        <el-form-item>
          <template #label>
            {{ translate('标签图标') }}
            <el-tooltip :content="translate('标签开启时生效')" effect="dark" placement="top">
              <vab-icon icon="question-line" />
            </el-tooltip>
          </template>
        ...
        <el-form-item>
          <template #label>
            {{ translate('分栏风格') }}
            <el-tooltip :content="translate('分栏布局时生效')" effect="dark" placement="top">
              <vab-icon icon="question-line" />
            </el-tooltip>
          </template>
          <el-select v-model="theme.columnStyle" :disabled="theme.layout !== 'column'">
            <el-option key="vertical" :label="translate('纵向')" value="vertical" />
            <el-option key="horizontal" :label="translate('横向')" value="horizontal" />
            <el-option key="card" :label="translate('卡片')" value="card" />
