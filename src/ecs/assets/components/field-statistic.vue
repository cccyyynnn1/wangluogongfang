...
<script setup lang="ts">
  import { updateSiteUnknowGroupByApi } from '~/src/api-ecs/site'
  ...
  const emit = defineEmits<{
    ...
    (e: 'changeShortcut', data: any): void
  }>()
  ...
  const aggregationsPayload1 = ref() // 回显数据
  ...
  const siteAggregations = async ({ list, num }: { list: string[]; num: number }) => {
    try {
      ...
      const { data } = await updateSiteUnknowGroupByApi({
  ...
  const tableColumn = [
    { fieldNameCn: '值', fieldNameEn: 'field' },
    { fieldNameCn: '次数', fieldNameEn: 'count' },
  ]
  ...
  <div class="field-statistic">
    <el-dialog v-model="visible" :before-close="handleClose" :title="props.title" width="1170px">
      ...
      <el-table v-loading="listLoading" :border="true" :data="listDate" row-key="id">
        ...
        <el-table-column :align="'center'" label="值" prop="field" show-overflow-tooltip />
        <el-table-column :align="'center'" label="次数" prop="count" show-overflow-tooltip width="200" />
        <template #empty>
