<script setup lang="ts">
  ...
  import { deleteAssetsApi, getAssetsListApi, exportAssetsApi } from '@/api-ecs/assets'

  import { getAssetsType, tableSearch } from '@/types'
  ...
  const handleDelete = ({ ...row }) => {
    ...
    if (delList.length > 0 && !row.deleteAll) {
      ...
      $baseConfirm('你确定要删除吗', null, async () => {
        const { msg } = await deleteAssetsApi({ ids, deleteAll: false, knowOrNot: '0' })
    ...
    if (!row.row && row.deleteAll) {
      $baseConfirm('你确定要删除所有数据吗', null, async () => {
        const { msg } = await deleteAssetsApi({ ids: '', deleteAll: true, knowOrNot: '0' })
        $baseMessage(msg, 'success', 'vab-hey-message-success')
        await getData()
      })
    }
  ...
  async function getData() {
    ...
    changeData()
    queryData.responsible = queryData.responsible ? queryData.responsible : undefined
...
<template>
  <div class="assets-known">
    ...
    >
      ...
      <el-table-column label="名称" prop="name" show-overflow-tooltip>
        <template #header="{ column }">
          <table-filter-tool
            v-model:filter-value="queryData.name"
            :column="column"
            filter-type="text"
            :tools="['filter']"
          />
      ...
      <el-table-column label="资产IP" prop="assetsIp" show-overflow-tooltip>
        <template #header="{ column }">
          <table-filter-tool
            v-model:filter-value="queryData.assetsIp"
            :column="column"
            filter-type="text"
            :tools="['filter']"
          />
      ...
      <el-table-column label="网络区域" prop="netArea" show-overflow-tooltip>
        <!-- <template #default="{ row }">
          {{ formatNetArea(row.netArea) }}
        </template> -->
      </el-table-column>
      <!-- <el-table-column label="在线状态" prop="isOnline" show-overflow-tooltip>
        <template #default="{ row }">
          {{ row.isOnline ? '在线' : '离线' }}
        </template>
      </el-table-column>
      <el-table-column label="最近在线时间" prop="lastTime" show-overflow-tooltip>
        <template #header="{ column }">
          <table-filter-tool
            v-model:filter-value="lastTime"
            :column="column"
            :filter-option="lastTimeOptions"
            filter-type="select"
            :tools="['filter']"
          />
        </template>
      ...
      <el-table-column label="负责人 " prop="responsible" show-overflow-tooltip>
        <template #header="{ column }">
          <table-filter-tool
            v-model:filter-value="queryData.responsible"
            :column="column"
            filter-type="text"
            :tools="['filter']"
          />
    ...
    />
    <!-- 添加资产 -->
    <add-assets v-if="showPage" :show-page="showPage" @on-close-event="showPage = false" @on-reflash="getData" />
    <!-- 导入资产 -->
    <ImportAssets
      v-if="showUpload"
      mode="know"
