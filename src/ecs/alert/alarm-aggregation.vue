<script setup lang="ts">
  ...
  import AlertDetailAggregation from './components/alert-detail-aggregation.vue'
  ...
  import { AlertItem, ShortcutListType, DowmloadType } from '@/types'
  ...
  const getPageData = () => {
    const arr = BASICDATA.splice(0, 100)
    alertInfo.list.push(...arr)
  }
  ...
  async function queryData(remark = true, updateChart = true) {
    ...
    try {
      ...
      const {
        data: { total, sumaryMap, resList, scrollId, aggOneCount },
      } = await getAlertApi(queryData)
      alertInfo.scrollId = scrollId
      alertInfo.aggOneCount = aggOneCount
      if (resList.length > 0) {
  ...
  function formatColum() {
    if (!userDisPlaysFiled.value) return
  ...
  function showAlertDetail(row: any) {
    const { startTime, endTime } = must_parames
    infoVal.value = { ...row, startTime, endTime }
  ...
  watch(
    ...
    () => {
      ...
      trendEchartOption.xAxis.data = date
      trendEchartOption.series.data = count
    }
  )
  watch(
    () => chartDetail.value,
  ...
  onMounted(async () => {
    if (route.query.attackIp) {
      ...
      const _sql = `源IP = "${route.query.attackIp}"`
      sqlComponentsRef.value?.changeSql(_sql)
    }
  ...
  <div class="alert-container flex-col">
    <div class="tools">
      ...
      <div class="tools-right" style="margin-bottom: 18px">
        <div class="action">
          <label>原始告警：</label>
          <el-switch v-model="queryForm.whiteType" :active-value="1" :inactive-value="0" @change="handleSwitch" />
          <el-button
          ...
          <div class="items">
            <div v-for="item in shortcutList" :key="item.id" class="item">
              <el-checkbox v-model="item.enable" @change="handleCheckboxChange(item.id)">
                <span>{{ item.label }} {{ item.relation }} {{ `"${item.value ?? ''}"` }}</span>
              </el-checkbox>
    ...
    <template v-if="!isAttackPerspective">
      <div>
        ...
        <vab-card class="access" shadow="hover" skeleton>
          <template #header>
            <p>
              <span class="log">
                <span class="risk_status" style="color: #6954f0">威胁告警</span>
                <span>&nbsp;-&nbsp;{{ alertInfo.aggOneCount || 0 }}条</span>
              </span>
      ...
      >
        <el-table-column v-if="showTableSelection" align="center" type="selection" width="55" />
        <el-table-column align="center" :index="curIndex" label="序号" type="index" width="60" />
        <el-table-column
        ...
        >
          ...
          <template #default="{ row }" v-else-if="item.fieldNameEn == 'threatLevel'">
            <span v-if="row.threatLevel" :class="['alert_tag', getThreatLevel(row.threatLevel)]">
