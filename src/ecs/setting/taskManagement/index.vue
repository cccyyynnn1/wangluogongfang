      ...
      <vab-query-form-top-panel>
        <el-form inline label-width="60px" :model="queryForm" @submit.prevent>
          <el-form-item label="任务名">
            <el-input v-model.trim="queryForm.taskName" clearable placeholder="请输入账号" />
          </el-form-item>
    ...
    <el-row :gutter="15">
      <el-col v-for="item in list" :key="item.id" :lg="6" :md="8" :sm="12" :xl="6" :xs="24">
        <vab-card class="task-item" shadow="hover">
          ...
          <el-image class="task-item-image" :src="require('@/assets/task_image/task.png')" />
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const handlePlay = (status) => {
        if (status) $baseMessage('模拟停用成功', 'success', 'vab-hey-message-success')
