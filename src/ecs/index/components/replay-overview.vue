...
<script setup lang="ts">
  ...
  import NetworkLayer from '@/ecs/retrieve/network-layer.vue'
  import AlarmAggregation from '@/ecs/alert/alarm-aggregation.vue'
  import LevelRule from '@/ecs/retrieve/components/level-rule.vue'
  import Publish from '@/ecs/retrieve/components/publish.vue'
  import AddSql from '@/ecs/site/site-session/components/add-sql.vue'
  ...
  import RetrieveHistory from '@/ecs/retrieve/components/history.vue'
  import SearchSql from '@/components/search-sql/index.vue'
  import RetrieveWorkSpace from '@/ecs/retrieve/components/work-space.vue'
  ...
  function formatColum(type: RetrieveIndexType) {
    const columnData = getTableColumn(queryForm.indexType)
  ...
  const reloadSearchHandle = (data: RetrieveHistoryItem, hasTime: boolean) => {
    let { searchSql } = data
    const { searchEdTime, searchStTime, indexType, inputSql, filterSqlArr } = data
    if (inputSql) {
      searchSql = inputSql
    } else {
      if (filterSqlArr) {
...
<template>
  <div class="packet-replay-overview">
    <div class="replayBack">
      <el-button @click="$emit('close')">
        <el-icon style="margin-right: 2px"><vab-icon icon="arrow-left-line" /></el-icon>
        返回
      </el-button>
    ...
    <el-tabs v-model="activeName" style="height: 100%">
                ...
                <el-select v-model="queryForm.indexType">
                  ...
                  >
                    ...
                    <span v-if="indexTypeCount" style="float: right">
                      {{
                        indexTypeCount?.[item.value.toString()] > 1
                          ? numberFormatter.format(indexTypeCount?.[item.value.toString()])
                          : '暂无数据'
                      }}
                    </span>
                  </el-option>
            ...
            <template v-for="item in tableColumn" :key="item.id">
              ...
              >
                ...
                <template v-else-if="item.fieldNameCn.includes('等级')" #default="{ row }">
                  <span v-if="row.ruleLevel" :class="['alert_tag', getThreatLevel(row.ruleLevel)]">
                    {{ row.ruleLevel }}
                  </span>
                </template>
              </el-table-column>
      ...
      <el-tab-pane label="网络层会话" lazy name="network-layer">
        <network-layer v-if="activeName === 'network-layer'" />
      </el-tab-pane>
      <el-tab-pane label="威胁告警" lazy name="alert">
        <alarm-aggregation v-if="activeName === 'alert'" />
      </el-tab-pane>
    </el-tabs>
    <!-- 表头字段配置 -->
    ...
    />
    <!-- 检索详情 -->
    <vab-dialog v-model="detailVisible" :destroy-on-close="true" title="详情" width="1125px">
    ...
    />
    <!-- 检索历史 -->
    <retrieve-history v-if="showHistory" v-model="showHistory" :space-id="spaceId" @reloadSearch="reloadSearchHandle" />
    <!-- 收藏夹 工作空间 -->
