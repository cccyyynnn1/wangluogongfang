...
<script setup lang="ts">
  ...
  G6.registerEdge('smooth', {
    draw(cfg, group) {
      ...
      const path = [
        ...
        ['C', startPoint!.x + hgap / 4, startPoint?.y, endPoint!.x - hgap / 2, endPoint?.y, endPoint?.x, endPoint?.y],
      ]
  ...
  const initChart = () => {
    container.value = document.getElementById('charts-container')
    ...
    if (typeof window !== 'undefined')
      window.onresize = () => {
        ...
        if (!container || !container.value.scrollWidth || !container.value.scrollHeight) return
