...
<script setup lang="ts">
  ...
  import { RetrieveIndexType, filedDataType, siteCurrentType } from '@/types'

  import { getPortByHostApi, updateUnknowSiteApi } from '~/src/api-ecs/site'
  ...
  const getResult = async (sessionId: number) => {
    ...
    formData.displayFields = allField
      .filter((item: { isDisplay: number }) => {
        return item.isDisplay
      })
      .filter(Boolean)
    // const hosts = props.currentItem.host
    // formData.siteName = props.currentItem.host
    changeHost()
    formData.siteUnknowId = sessionId
    options.value = allField
      .filter((item) => item.isDeleted != 1)
      .filter(Boolean)
      .filter((item: any) => item.fieldNameEn != 'position')
    allHttpField.value = allField.filter((item: { isDisplay: number }) => {
      return !item.isDisplay
    })
    initOverlay()
  }
  ...
  const getHttpField = async () => {
    allHttpField.value = getTableColumn(30)
    options.value = allHttpField.value
      .filter((item: any) => item.isDeleted != 1)
      .filter(Boolean)
      .filter((item: any) => item.fieldNameEn != 'position')
    formData.displayFields = allHttpField.value.filter((item: { isDisplay: number }) => {
      return item.isDisplay
    })
    allHttpField.value = allHttpField.value.filter((item: { isDisplay: number }) => {
      return !item.isDisplay
    })
  }
  ...
  // 格式化host
  const changeHost = () => {
    const hosts = `${props.currentItem.host}${props.currentItem.serverIp ? `,${props.currentItem.serverIp}` : ''}`
    ...
    arr.forEach((item: any) => {
      ...
      if (has1 >= 0 && has2 >= 0) {
        ...
        if (!hostArr.value.some((item: any) => item.host === obj.host)) {
          hostArr.value.push(obj)
        }
      }
    })

    hostArr.value.forEach((_, index) => {
      handleSearch(0)
    })
  }
  ...
  const submitForm = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid, fields) => {
      if (valid) {
        ...
        const { msg } = await updateUnknowSiteApi({
          ...formData,
  ...
  watch(
    ...
    () => {
      ...
      if (props.showEditSite) {
        if (mode.value == 'edit') {
          const id = props.currentItem.id as number
          if (props.source == 'un-konw-site') {
            getHttpField()
          }
          getResult(id)
        } else {
  ...
  const handleSearch = async (_index: number) => {
    ...
    try {
      ...
      const { data } = await getPortByHostApi({ ...queryForm, searchStr: str, isUnknowSite: true })
...
<template>
  <div class="edit-site">
    <el-drawer v-model="visible" :before-close="handleClose" size="85%" :title="title">
