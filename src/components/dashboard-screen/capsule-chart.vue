<script setup lang="ts">
  ...
  const calcCapsuleLengthAndLabelData = () => {
    ...
    capsuleLength.value = newcapsuleValue.map((v: any) => (maxValue ? v / maxValue : 0))
    const oneFifth = maxValue / 5
    const newlabelData = Array.from(new Set(new Array(6).fill(0).map((v, i) => Math.ceil(i * oneFifth))))
  ...
  <div class="dv-capsule-chart">
    <template v-if="mergedConfig">
      ...
      <div class="capsule-container">
        <div v-for="(capsule, index) in capsuleLength" :key="index" class="capsule-item">
          <div
      ...
      <div v-if="mergedConfig.unit" class="unit-text">
        {{ mergedConfig.unit }}
      </div>
    </template>
  </div>
