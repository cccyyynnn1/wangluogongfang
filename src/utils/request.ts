...
import { baseURL, contentType, debounce, messageName, requestTimeout, statusName, successCode } from '@/config'
import qs from 'qs'
...
//   AxiosError,
// } from 'axios'
// import type { ResponseData } from '../types/constant'
...
const requestConf = (config: any) => {
  ...
  if (window.location.href.includes('asset-access-insights')) {
    config.headers['authId'] = routeParams.get('authId')
    config.headers['timestamp'] = routeParams.get('timestamp')
    config.headers['token'] = routeParams.get('token')
  } else {
    // 规范写法 不可随意自定义
    if (token) config.headers['Cookies'] = `EcsSessionId=${token}`
    if (token) config.headers!['Authorization'] = `Bearer ${token}`
    cookie.set('Cookies', `EcsSessionId=${token}`, { expires: 365 })
  }
  if (config.data && config.headers!['Content-Type'] === 'application/x-www-form-urlencoded;charset=UTF-8')
    config.data = qs.stringify(config.data)
  if (debounce.some((item) => config.url!.includes(item))) loadingInstance = gp.$baseLoading()
  return config
}
...
const handleData = async ({ config, data, status, statusText }: any) => {
  ...
  switch (code) {
    ...
    case 10010:
      return gp.$baseMessage(data.msg, 'error', 'vab-hey-message-error', false)
      break
  }
  ...
  const errMsg = `${
    data && data[messageName] ? data[messageName] : CODE_MESSAGE[code] ? CODE_MESSAGE[code] : data.data
  }`
  if (
    config.url !== '/v3/eht/public/license' ||
    (config.url === '/v3/ecsPlatform/public/forward' && !config.data?.includes('firewallPolicy/page')) //防火墙策略偶尔报错取消报错提示
  ) {
...
instance.interceptors.response.use(
  ...
  (error) => {
    ...
    if (code === 'ERR_CANCELED') return Promise.reject('已取消请求')
    if (response === undefined) {
      if (loadingInstance) loadingInstance.close()
      gp.$baseMessage('请刷新后重新尝试', 'warning', 'vab-hey-message-warning', false)
      return {}
