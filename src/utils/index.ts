...
export function translateDataToTree(data: any[]) {
  const parent = data.filter(
    (value: { parentId: string | null }) => value.parentId === 'undefined' || value.parentId === null
  )
...
export function translateTreeToData(data: any[]) {
  ...
  data.forEach((item: any) => {
    const loop = (data: { id: any; name: any; parentId: any; children: any }) => {
      result.push({
        id: data.id,
        name: data.name,
...
export function useCopy(data: string) {
  if (data) {
    const oInput = document.createElement('textarea') //创建一个input标签
    ...
    oInput.remove() //只是用一下input标签的特性，实际并不需要显示，所以这里要隐藏掉
    gp.$baseMessage('复制成功', 'success')
  }
}
 ...
 * @returns void
 */
export function useTableCopy(row: any, column: any, cell: any, event: any, arr: any) {
  const list = arr?.length > 0 ? arr : undefined
  let value = undefined
  const target = column.property
  if (target) {
    value = row[target]
  }
  const data = { value, cell, event, list }
  VueEvent.emit('useContextMenu', data)
}
...
export const fetchTimeOut = (timeout = 60000) => {
