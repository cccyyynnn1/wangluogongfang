 ...
 * @returns {[]}
 */
export function filterRoutes(routes: VabRouteRecord[], rolesControl: boolean, baseUrl = '/'): VabRouteRecord[] {
  return routes
    .filter((route: VabRouteRecord) => (rolesControl && route.meta.guard ? hasPermission(route.meta.guard) : true))
 ...
 * @returns {*} matched
 */
export function handleMatched(routes: VabRouteRecord[], path: string): VabRouteRecord[] {
  return routes
    .filter((route: VabRouteRecord) => (route?.childrenPathList || []).indexOf(path) + 1)
    .flatMap((route: VabRouteRecord) => (route.children ? [route, ...handleMatched(route.children, path)] : [route]))
}
